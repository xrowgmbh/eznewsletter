<form name="editform" onsubmit="return eznewsletterCheckSendDate();" id="editform" enctype="multipart/form-data" method="post" action={concat( '/content/edit/', $object.id, '/', $edit_version, '/', $edit_language|not|choose( concat( $edit_language, '/' ), '/' ), $is_translating_content|not|choose( concat( $from_language, '/' ), '' ) )|ezurl}>
...
function eznewsletterCheckSendDate()
{ldelim}
    var sendDateYear = document.getElementById( 'newsletter_send_date_year' );
    var sendDateMonth = document.getElementById( 'newsletter_send_date_month' );
    var sendDateDay = document.getElementById( 'newsletter_send_date_day' );
    var sendDateHour = document.getElementById( 'newsletter_send_date_hour' );
    var sendDateMinute = document.getElementById( 'newsletter_send_date_minute' );
    if ( sendDateYear && sendDateMonth && sendDateDay && sendDateHour && sendDateMinute )
    {ldelim}
        var sendTime = new Date( sendDateMonth.value + ' ' + sendDateDay.value + ', ' + sendDateYear.value + ' ' + sendDateHour.value + ':' + sendDateMinute.value + ':00' );
        var nowDate = new Date();
        var nowMonth = nowDate.getMonth();
        nowMonth = nowMonth+1;

        if ( sendTime.getTime() < nowDate.getTime() )
        {ldelim}
            var change = confirm( '{'You entered a date in the past. Do you want to set it to current date and continue publishing?'|i18n('design/newsletter/edit/article')}' );
            if ( change )
            {ldelim}
                sendDateYear.value = nowDate.getFullYear();
                sendDateMonth.value = nowDate.getMonth() + parseInt(1);
                sendDateDay.value = nowDate.getDate();
                sendDateHour.value = nowDate.getHours();
                sendDateMinute.value = nowDate.getMinutes();
                return true;
            {rdelim}
            else
            {ldelim}
                return confirm( '{'Do you want continue publishing?'|i18n('design/newsletter/edit/article')}' );
            {rdelim}
        {rdelim}
    {rdelim}
    return true;
