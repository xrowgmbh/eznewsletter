...
{def $base_uri=concat( '/newsletter/list_archive/', $newsletter_type.id)}

<form name="newsletter_list" method="post" action={$base_uri|ezurl}>

<div class="context-block">
{* DESIGN: Header START *}<div class="box-header"><div class="box-tc"><div class="box-ml"><div class="box-mr"><div class="box-tl"><div class="box-tr">
<h2 class="context-title">{'Newsletter archive'|i18n( 'design/eznewsletter/list_archive' )} [ {$newsletter_type.name} ]</h1>
...
<tr>
    ...
    <th>{'Name'|i18n( 'design/eznewsletter/list_archive' )}</th>
    <th>{'Creator'|i18n( 'design/eznewsletter/list_archive' )}</th>
    <th>{'Created'|i18n( 'design/eznewsletter/list_archive' )}</th>
    <th>{'Send date'|i18n( 'design/eznewsletter/list_archive' )}</th>
    <th>{'Send status'|i18n( 'design/eznewsletter/list_archive' )}</th>
    <th>{'Status'|i18n( 'design/eznewsletter/list_archive' )}</th>
    <th>{'# sent'|i18n( 'design/eznewsletter/list_archive' )}</th>
    <th>{'# read'|i18n( 'design/eznewsletter/list_archive' )}</th>
    <th>&nbsp;</th>
</tr>
{foreach fetch( newsletter, newsletter_list_by_type, hash( type_id, $newsletter_type.id, is_sent, true(), offset, $offset, limit, $number_of_items, $grouping, true() ) ) as $newsletter
...
<tr class="{$seq}">
    ...
    <td><a href={$newsletter.contentobject.main_node.url_alias|ezurl}>
        <img src={'preview.gif'|ezimage} title="{'Preview newsletter'|i18n( 'design/eznewsletter/list_archive' )}">
        </a>
    </td>
    </tr>
{/foreach}
...
{include name=navigator
         ...
         page_uri=$base_uri
         item_count=fetch( newsletter, newsletter_list_by_type, hash( type_id, $newsletter_type.id, is_sent, true(), offset, -1, limit, -1, $grouping, true() ) )|count()
