...
<h2 class="context-title">{'Newsletter in progress'|i18n( 'design/eznewsletter/list_inprogress' )} [ {$newsletter_type.name} ]</h1>
...
<tr>
    ...
    <th>{'Name'|i18n( 'design/eznewsletter/list_inprogress' )}</th>
    <th>{'Creator'|i18n( 'design/eznewsletter/list_inprogress' )}</th>
    <th>{'Created'|i18n( 'design/eznewsletter/list_inprogress' )}</th>
    <th>{'Send date'|i18n( 'design/eznewsletter/list_inprogress' )}</th>
    <th>{'Send status'|i18n( 'design/eznewsletter/list_inprogress' )}</th>
    <th>{'Status'|i18n( 'design/eznewsletter/list_inprogress' )}</th>
    <th>{'# sent'|i18n( 'design/eznewsletter/list_inprogress' )}</th>
    <th>{'# read'|i18n( 'design/eznewsletter/list_inprogress' )}</th>
    <th>&nbsp;</th>
</tr>
{foreach fetch( newsletter, newsletter_list_by_type, hash( type_id, $newsletter_type.id, is_sent, false(), offset, $offset, limit, $number_of_items, $grouping, true() ) ) as $newsletter
...
<tr class="{$seq}">
    ...
    <td><a href={$newsletter.contentobject.main_node.url_alias|ezurl}>
        <img src={'preview.gif'|ezimage} title="{'Preview newsletter'|i18n( 'design/eznewsletter/list_inprogress' )}">
        </a>
    </td>
    </tr>
{/foreach}
...
{include name=navigator
         ...
         page_uri=$base_uri
         item_count=fetch( newsletter, newsletter_list_by_type, hash( type_id, $newsletter_type.id, is_sent, false(), offset, -1, limit, -1, $grouping, true() ) )|count()
