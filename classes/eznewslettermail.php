...
//
// Definition of eZNewsletterMail class
//
...
/*!
  \class eZNewsletterMail eznewslettermail.php
  \brief Mail handler
...
class eZNewsletterMail
{
    const MAIL_REGEXP = '([0-9a-zA-Z]([-.\w]*[0-9a-zA-Z_])*@(((([0-9a-zA-Z])+([-\w]*[0-9a-zA-Z])*\.)+[a-zA-Z]{2,9})|(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)))';

    /*!
      Constructs a new eZNewsletterMail object.
    */
    ...
    */
    ...
    /*!
        Returns composed message text
    */
    function mobileTextList()
    {
        return $this->composeEmailItems( $this->MobileElements, false, 'mobile', true );
    }
    ...
    */
    ...
    {
        $text = eZNewsletterMail::composeEmailName( $this->From );
        if ( !$convert )
            return $text;
    ...
    */
    ...
    /*!
      ...
      \sa addReceiver, setReceiverElements
    */
    function setMobile( $mobile, $name = false )
    {
        $this->MobileElements = array( array( 'mobile' => $mobile ) );
    }
    ...
    */
    ...
    function setDateTimestamp( $newDate )
    {
        $this->Date = $newDate;
    }
    ...
    */
    ...
    {
        if ( preg_match( "/([^<]+)<" . eZNewsletterMail::MAIL_REGEXP . ">/", $text, $matches ) )
        {
            $email = $matches[2];
            $name = $matches[1];
        }
    ...
    */
    ...
    {
        $res = ereg( eZNewsletterMail::MAIL_REGEXP, $address, $email );
        if ( $res )
            return $email[0];
        else
            return 0;
    ...
    */
    static function composeEmailName( $item, $key = false )
    {
        if ( $key !== false and
             isset( $item[$key] ) )
            return $item[$key];
    ...
    */
    function composeEmailItems( $items, $join = true, $key = false, $convert = true )
    {
        ...
        {
            $textElements[] = eZNewsletterMail::composeEmailName( $item, $key );
        }
    ...
    */
    function headers( $parameters = array() )
    {
        ...
        if ( $this->Date )
            {
        $headers[] = array( 'name' => 'Date', 'content' => date( 'r', $this->Date ) );
            }
        else
        {
        $headers[] = array( 'name' => 'Date', 'content' => date( 'r' ) );
            }
        $headerNames[] = 'date';
        }
